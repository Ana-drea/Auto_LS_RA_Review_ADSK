...
for %%i in (chs, cht, fra, deu, ita,jpn,kor) do pslcmd.exe /openproject:%folder%\%%i\CurrentReleaseLpu\%%i_All_Nastran_InCAD.lpu /runmacro=Remove_langs_from_project.bas & pslcmd.exe /openproject:%folder%\%%i\LastReleaseLpu\%%i_All_Nastran_InCAD.lpu /runmacro=Remove_langs_from_project.bas
...
pause
for %%i in (chs, cht, fra, deu, ita,jpn,kor) do cSmoke_CommandLine --test=RA --user=andrea.zhou@rws.com --product=NSTRN --language=%%i --release=2023 --build=%build% --previouslpu="%folder%\%%i\LastReleaseLpu" --currentlpu="%folder%\%%i\CurrentReleaseLpu" --mappingfile="%folder%\%%i\CurrentReleaseLpu\mapping.txt" --enablelog=true
pause
