...
echo start removing extra string lists except Helius from lpu, this may take a while...
for %%i in (chs, cht, fra, deu, ita,jpn,kor,ptg,esn) do pslcmd.exe /openproject:%folder%\%%i\CurrentReleaseLpu\%%i_All_Moldflow.lpu /runmacro=KeepOnlyHeliusInStringLists.bas & pslcmd.exe /openproject:%folder%\%%i\LastReleaseLpu\%%i_All_Moldflow.lpu /runmacro=KeepOnlyHeliusInStringLists.bas
echo you can go and check before proceeding to connect to ADSK server and create creating for_review lpus
pause
for %%i in (chs, cht, fra, deu, ita,jpn,kor,ptg,esn) do cSmoke_CommandLine --test=RA --user=andrea.zhou@rws.com --product=ACMPAN --language=%%i --release=2023 --build=%build% --previouslpu="%folder%\%%i\LastReleaseLpu" --currentlpu="%folder%\%%i\CurrentReleaseLpu" --mappingfile="%folder%\%%i\CurrentReleaseLpu\mapping.txt" --enablelog=true
pause
